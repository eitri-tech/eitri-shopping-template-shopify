...
} from 'eitri-shopping-shopify-shared'
...
export default function Addresses(props) {
	...
	const saveMutation = useMutation<
		{ address: CustomerAddress | null; userErrors: CustomerUserError[] },
		Error,
		{ mode: string; editingId: string | null; addressInput: CustomerAddressInput; isFirst: boolean }
	>({
	...
	const deleteMutation = useMutation<{ success: boolean; userErrors: CustomerUserError[] }, Error, string>({
	...
	const openEdit = address => {
		setForm({
			...
			territoryCode: address.countryCode || 'BR'
		})
	...
	const handleSave = () => {
		...
		const addressInput = {
			...
			territoryCode: form.territoryCode || 'BR'
		}
	...
	return (
		<View className='flex flex-col bg-white rounded-lg border border-gray-200 overflow-hidden'>
			<View className='flex flex-col p-4 gap-1'>
				...
				{address.country && <Text className='text-sm text-gray-400'>{address.country}</Text>}
				{address.phoneNumber && <Text className='text-sm text-gray-400'>{address.phoneNumber}</Text>}
			</View>
				...
				{!isDefault && (
					<View
						...
						<Text className='text-sm text-gray-500'>
							{t('account.addresses.setDefault', 'Tornar principal')}
						</Text>
					</View>
				)}
